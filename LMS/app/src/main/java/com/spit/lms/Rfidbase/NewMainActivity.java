...
//BB44F26E12E74FA4867FD10BD41C4CBD,BF07994FB554493B97A46B39083C8A61
    ...
    public static String getLocationString(String first, String last) {
        String tempLast = last;

        try {
            String base = locationHashMap.get(tempLast).getLocationName();

            while (locationHashMap.get(tempLast) != null && locationHashMap.get(tempLast).getFatherRoNo() != first) {

                tempLast = locationHashMap.get(tempLast).getFatherRoNo();
                Log.i("fatherno", "fatherno " + tempLast );

                if(locationHashMap.get(tempLast).getFatherRoNo() == null)
                    break;

                base = locationHashMap.get(tempLast).getLocationName() + "->" + base;
                Log.i("base", "base " + base);
    ...
    public static String getCategoryString(String first, String last) {
        String tempLast = last;

        try {
            String base = categoryHashMap.get(tempLast).getCategoryName();

            while (categoryHashMap.get(tempLast) != null && categoryHashMap.get(tempLast).getFatherRoNo() != first) {

                tempLast = categoryHashMap.get(tempLast).getFatherRoNo();
                Log.i("fatherno", "fatherno " + tempLast );

                if(categoryHashMap.get(tempLast).getFatherRoNo() == null)
                    break;

                base = categoryHashMap.get(tempLast).getCategoryName() + "->" + base;
                Log.i("base", "base " + base);
    ...
    public static boolean isInternetAvailable() {
        try {
            ...
            InetAddress ipAddr = InetAddress.getByName(SharedPrefsUtils.getStringPreference(MainActivity.mContext, "BASE_URL") );

            Log.i("data", "data " + ipAddr);
    ...
    public static boolean isConnected()  {
        ...
        try {
            ...
            try{
                URL myUrl = new URL(SharedPrefsUtils.getStringPreference(MainActivity.mContext, "BASE_URL") + "/search?page=0&query=");
                HttpURLConnection connection = (HttpURLConnection) myUrl.openConnection();
                connection.setConnectTimeout(400);
                connection.connect();
                int code = connection.getResponseCode();

                Log.i("conn.getResponseCode()", "conn.getResponseCode()" + code);
                if(code == 200) {
                    return true;
                }
                return false;
           ...
           // String command = "ping -c 1 " + SharedPrefsUtils.getStringPreference(MainActivity.mContext, "BASE_URL");
